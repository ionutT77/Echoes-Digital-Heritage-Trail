import React, { useEffect, useState } from 'react';
import { Marker } from 'react-leaflet';
import { Icon } from 'leaflet';
import useMapStore from '../../stores/mapStore';
import { isWithinProximity } from '../../utils/distance';

function NodeMarker({ node }) {
  const userLocation = useMapStore((state) => state.userLocation);
  const discoveredNodes = useMapStore((state) => state.discoveredNodes);
  const setSelectedNode = useMapStore((state) => state.setSelectedNode);
  const isCustomPathMode = useMapStore((state) => state.isCustomPathMode);
  const customPathSelectedNodes = useMapStore((state) => state.customPathSelectedNodes);
  const toggleCustomPathNode = useMapStore((state) => state.toggleCustomPathNode);
  const [isUnlocked, setIsUnlocked] = useState(false);

  useEffect(() => {
    if (userLocation) {
      const unlocked = isWithinProximity(
        userLocation.lat,
        userLocation.lng,
        node.latitude,
        node.longitude,
        node.proximityRadius
      );
      setIsUnlocked(unlocked);
    }
  }, [userLocation, node]);

  const isDiscovered = discoveredNodes.has(node.id);
  const isSelectedForCustomPath = customPathSelectedNodes.has(node.id);

  const handleMarkerClick = () => {
    if (isCustomPathMode) {
      // In custom path mode, toggle selection
      toggleCustomPathNode(node.id);
      console.log('Toggled custom path node:', node.title);
    } else {
      // Normal mode, open node modal
      console.log('Marker clicked:', node.title);
      setSelectedNode(node);
    }
  };

  const markerIcon = React.useMemo(() => {
    let iconUrl;
    
    if (isSelectedForCustomPath) {
      // Green marker for selected nodes in custom path mode
      iconUrl = 'data:image/svg+xml;base64,';
    } else if (isDiscovered) {
      // Orange/amber marker for discovered nodes
      iconUrl = 'data:image/svg+xml;base64,';
    } else {
      // Brown/heritage marker for undiscovered nodes
      iconUrl = 'data:image/svg+xml;base64,';
    }

    return new Icon({
      iconUrl: iconUrl,
      iconSize: [32, 48],
      iconAnchor: [16, 48],
      popupAnchor: [0, -48],
    });
  }, [isDiscovered, isSelectedForCustomPath]);

  return (
    <Marker
      position={[node.latitude, node.longitude]}
      icon={markerIcon}
      zIndexOffset={isSelectedForCustomPath ? 2000 : isDiscovered ? 1000 : 0}
      className={isUnlocked ? 'marker-pulse unlocked-marker' : ''}
      eventHandlers={{
        click: handleMarkerClick
      }}
    />
  );
}

export default NodeMarker;
